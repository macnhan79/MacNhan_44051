...
char map[50][50]; // Mảng 2 chiỿu các ký tự.
...
// Hàm dịch chuyển con trỿ đến tỿa độ x, y.
    ...
    for (int i = 0; i < 30; i++) {
        ...
        for (int j = 0; j < 30; j++) {
            ...
            if (j == 0 || j == 29) {
                ...
                cout << map[i][j];
                textcolor(7); // Trở lại màu bình thưỿng.
            }                /* IN XE COMPUTER */
            else if (map[i][j] == '!') {
                //map[i][j] = ' '; // Bỿ đi ký tự '!'
                textcolor(200);
...
void DiChuyenXe(int &x, int &y) // x, y là tỿa độ hiện tại của xe.
{
    /* 
    ...
    if (GetAsyncKeyState(VK_LEFT)) {
        if (y > 2) {
            // Ŀang so sánh xem có bị trùng với xe tĩnh & xe động hay không ?
...
int main() {
    ...
    int x = 20, y = 15; // Tỿa độ của xe Player.
    int x1 = 2, y1; // Tỿa độ xe chướng ngại vật động.
    int x2 = 5, y2 = 10; // Tỿa độ xe chướng ngại vật tĩnh.
